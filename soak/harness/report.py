...
def _resource_table(results: Results) -> list[str]:
    lines = [
        "## Resources",
        "",
        "| Unit | RSS start | RSS end | Slope | FDs start | FDs end | CLOSE-WAIT peak/end | FIN-WAIT-2 peak | Zombies peak/end | Restarts in window |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for unit, r in results.resources.items():
        lines.append(
            ...
            f"{r.fin_wait_2_max} | {r.zombies_max}/{r.zombies_end} | {r.restarts_in_window} |"
        )
    lines.append("")
    return lines
